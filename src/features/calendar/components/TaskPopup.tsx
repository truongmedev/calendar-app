  ...
  if (popup.mode === 'menu') {
    return createPortal(
      <div ref={ref} className="calendar-context-menu" onContextMenu={(e) => e.preventDefault()}>
        <button autoFocus type="button" onClick={() => dispatch({ type: 'open', popup: { ...popup, mode: 'edit', point: undefined } })}>Sửa (Edit)</button>
        <button type="button" className="calendar-context-delete" onClick={() => dispatch({ type: 'delete', id: task.id })}>Xóa (Delete)</button>
      </div>, document.body);
  }
  ...
  return createPortal(
    <div ref={ref} className="calendar-modal" role="dialog" aria-label={heading}>
      ...
      {popup.mode === 'view' ? <TaskDetails task={task} />
        : <TaskForm task={task} editing={popup.mode === 'edit'} onSave={(updated) => dispatch({ type: 'save', task: updated })} />}
    </div>, document.body);
}
