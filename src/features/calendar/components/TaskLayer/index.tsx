import './style.css';
import { memo, PointerEvent, RefObject, useCallback, useMemo, useReducer } from 'react';
...
function TaskLayer({ days, scrollerRef }: Props) {
  ...
  const popupTask = state.draft?.id === state.popup?.taskId
    ? state.draft
    : state.tasks.find((task) => task.id === state.popup?.taskId);

  const startCreatingTask = (event: PointerEvent<HTMLDivElement>, date: string) => {
    if (!event.isPrimary || event.button !== 0) return;
    const offset = event.clientY - event.currentTarget.getBoundingClientRect().top;
    const startMinute = clamp(Math.round(offset / PIXELS_PER_MINUTE), 0, MINUTES_PER_DAY - 1);
  ...
  return (
    <>
      <div className="calendar-layers" style={{ gridTemplateColumns: `repeat(${days.length}, minmax(0, 1fr))` }}>
        {dayLayouts.map(({ day, tasks, layouts }) => (
          <div
            ...
            onPointerDown={(event) => startCreatingTask(event, day.date)}
          >
            {tasks.map((task) => {
              const layout = layouts.get(task.id) ?? { column: 0, columns: 1, conflict: false };
              const selected = state.popup?.taskId === task.id
                && (state.popup.mode === 'view' || state.popup.mode === 'create');
              return (
                <TaskCard
                  key={task.id}
                  task={task}
      ...
      {state.popup && popupTask && (
        <TaskPopup
          key={`${state.popup.mode}-${popupTask.id}`}
          popup={state.popup}
          task={popupTask}
          dispatch={dispatch}
        />
